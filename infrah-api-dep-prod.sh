...
initialize_configuration() {
    # Traverse up the directory tree to find globalenvprod.config
    local dir
    dir=$(pwd)
    while [[ "$dir" != "/" ]]; do
        if [[ -f "$dir/globalenvprod.config" ]]; then
            # shellcheck source=/dev/null
            source "$dir/globalenvprod.config"
            return 0
        fi
        dir=$(dirname "$dir")
    done

    log_error "globalenvprod.config not found"
    exit 1
}
